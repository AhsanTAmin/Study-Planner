...
public class EarliestDeadlineStrategyTest {

    @Test
    public void shouldSortTasksByEarliestDeadline() {
        ...
        SchedulingStrategy strategy = new EarliestDeadlineStrategy();
        List<Task> result = strategy.generatePlan(tasks);

        assertEquals("Task B", result.get(0).getName());
        assertEquals("Task C", result.get(1).getName());
        assertEquals("Task A", result.get(2).getName());
    }
}
